...
class EventParticipantController {
   ...
   * @param {Response} ctx.response
   */
  async inscription_training ({ request, response }) {
    ...
    const participantAdded = await Participant.findByOrFail(
      'entity_id',
      entity_id
    )
  ...
  async store ({ request, response }) {
    ...
    const event_organizator = entity
      .toJSON()
      .organizators.find(
        (organizator) => organizator.id === parseInt(event_id)
      )
  ...
  async searchParticipant ({ params, request, response, view }) {
    try {
      ...
      const participant = await Entity.findByOrFail('cpf', params.cpf_email)
      ...
      const event_organizator = participant
        .toJSON()
        .organizators.find(
          (organizator) => organizator.id === parseInt(params.event_id)
        )
      ...
      if (event_participant !== undefined) {
        if (!event_participant.pivot.assistant) {
          return response.status(200).send({
            error: {
              title: 'Aviso!',
              message: 'O usuário informado ja está cadastrado',
              type: 'error'
            }
          })
        }
      ...
      if (event_organizator !== undefined) {
        return response.status(200).send({
          error: {
            title: 'Aviso!',
            message: 'O usuário informado ja é um organizador cadastrado',
            type: 'error'
          }
        })
      }
