...
const Help = use("App/Helpers/create_entity_helper");
...
class EntityController {
   ...
   */
  async index() {
    const entity = await Entity.query().with("file").with("families").fetch();
  ...
  async store({ request }) {
    ...
    if (invite) {
      ...
      Kue.dispatch(JobCreate.key, { entity, OAuth }, { attempts: 5 });
    }
  ...
  async update_netsuite({ params, request, response }) {
    try {
      ...
      if (data.organization_id) {
        const organization = await Organization.findBy(
          "netsuite_id",
