...
define(["N/runtime", "N/record", "N/http"], function (runtime, record, http) {
  ...
  function afterSubmit(scriptContext) {
    ...
    const dashboard_id = data.getValue({
      ...
      if (is_person === "T") {
        http.post({
          ...
          body: {
            ...
            organization_id: data.getValue({
              fieldId: "custentity_rsc_igreja",
            }),
            cmn_hierarchy_id:
              data.getValue({ fieldId: "custentity_cmn_hierarchy_id" }) || "0",
            mu_hierarchy_id:
              data.getValue({ fieldId: "custentity_mu_hierarchy_id" }) || "0",
            crown_hierarchy_id:
              data.getValue({ fieldId: "custentity_crown_hierarchy_id" }) ||
              "0",
            mp_hierarchy_id:
              data.getValue({ fieldId: "custentity_mp_hierarchy_id" }) || "0",
            ffi_hierarchy_id:
              data.getValue({ fieldId: "custentity_ffi_hierarchy_id" }) || "0",
            gfi_hierarchy_id:
              data.getValue({ fieldId: "custentity_gfi_hierarchy_id" }) || "0",
            pg_hab_hierarchy_id:
              data.getValue({ fieldId: "custentity_pg_hab_hierarchy_id" }) ||
              "0",
            pg_yes_hierarchy_id:
              data.getValue({ fieldId: "custentity_pg_yes_hierarchy_id" }) ||
              "0",
            user_legacy: true,
          },
        });
        ...
        http.post({
          ...
          body: {
            ...
            type:
              data.getText({ fieldId: "custentity_rsc_categoriacliente" }) ||
              "",
            corporate_name:
              data.getText({ fieldId: "custentity_enl_legalname" }) || "",
            fantasy_name:
              data.getText({ fieldId: "custentity_enl_legalname" }) || "",
            email: data.getValue({ fieldId: "email" }) || "",
            cnpj: data.getValue({ fieldId: "custentity_enl_cnpjcpf" }) || "",
            inscricao_estadual:
              data.getValue({ fieldId: "custentity_enl_ienum" }) || "",
            password: "udf123",
            phone: data.getValue({ fieldId: "phone" }) || "",
            alt_phone: data.getValue({ fieldId: "altphone" }) || "",
          },
    ...
    if (type === "edit") {
      if (is_person === "T") {
        ...
        http.put({
          url:
            "http://apieventos.udf.org.br/netsuite_organization/" + netsuite_id,
          body: {
            type:
              data.getText({ fieldId: "custentity_rsc_categoriacliente" }) ||
              "",
